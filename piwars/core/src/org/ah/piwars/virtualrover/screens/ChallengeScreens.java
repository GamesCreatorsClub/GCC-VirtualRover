    ...
    public <T extends ChallengeScreen> T getChallengeScreen(String mapId) {
        ...
        if ("PiNoon".equals(mapId)) {
            if (piNoonScreen == null) {
                piNoonScreen = new PiNoonScreen(mainGame, platformSpecific, assetManager, soundManager, modelFactory, serverCommunicationAdapter, console);
            }
            challengeScreen = piNoonScreen;
        } else if ("EcoDisaster".equals(mapId)) {
            if (ecoDisasterScreen == null) {
                ecoDisasterScreen = new EcoDisasterScreen(mainGame, platformSpecific, assetManager, soundManager, modelFactory, serverCommunicationAdapter, console);
            }
            challengeScreen = ecoDisasterScreen;
        } else if ("CanyonsOfMars".equals(mapId)) {
            if (canyonsOfMarsScreen == null) {
                canyonsOfMarsScreen = new CanyonsOfMarsScreen(mainGame, platformSpecific, assetManager, soundManager, modelFactory, serverCommunicationAdapter, console);
            }
            challengeScreen = canyonsOfMarsScreen;
        } else if ("StraightLineSpeedTest".equals(mapId)) {
            if (straightLineSpeedTestScreen == null) {
                straightLineSpeedTestScreen = new StraightLineSpeedTestScreen(mainGame, platformSpecific, assetManager, soundManager, modelFactory, serverCommunicationAdapter, console);
            }
            challengeScreen = straightLineSpeedTestScreen;
        } else if ("BlastOff".equals(mapId)) {
            if (blastOffScreen == null) {
                blastOffScreen = new BlastOffScreen(mainGame, platformSpecific, assetManager, soundManager, modelFactory, serverCommunicationAdapter, console);
            }
            challengeScreen = blastOffScreen;
        } else if ("MineSweeper".equals(mapId)) {
            if (mineSweeperScreen == null) {
                mineSweeperScreen = new MineSweeperScreen(mainGame, platformSpecific, assetManager, soundManager, modelFactory, serverCommunicationAdapter, console);
            }
